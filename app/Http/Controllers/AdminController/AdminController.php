...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        return view('admin.dashboard', compact('nombreComptes','nombrePaiements', 'montantPaiements', 
'nombreClients', 'listeComptes'));
    }
...
public function supprimerCompte($id) {
    ...
    if ($compte) {
        ...
        $conversations = Conversation::where('user_id', $id)->orWhere('admin_id', $id)->get();
        foreach ($conversations as $conversation) {
            Message::where('conversation_id', $conversation->id)->delete();
        }

        // Supprimer les conversations associées à l'utilisateur
        Conversation::where('user_id', $id)->orWhere('admin_id', $id)->delete();
